...
export function InteractiveWeeklyCalendarGrid({
  ...
  const scheduleStartDate = fromUTC(schedule.startDate);
  const scheduleEndDate = fromUTC(schedule.endDate);

  const [currentDate, setCurrentDate] = useState<Date>(() => {
    const today = new Date();
    if (today < scheduleStartDate) return scheduleStartDate;
    if (today > scheduleEndDate) return scheduleEndDate;
    return today;
  });
  ...
  // Dynamic hour height to fill viewport cleanly without unnecessary dead space
  const hourHeight = gridHoursLength <= 6 ? 56 : gridHoursLength <= 8 ? 48 : gridHoursLength <= 11 ? 40 : 34;
      ...
      {view === "week" && (
        <div className="flex-1 min-h-0 border border-border/80 rounded-xl overflow-y-auto overflow-x-auto scrollbar-thin bg-background relative shadow-2xs">
          <div className="min-w-[700px] flex flex-col h-full">
            ...
            <div className="grid grid-cols-8 divide-x divide-border/70 border-b border-border/70 bg-muted/95 backdrop-blur-xs sticky top-0 z-20 shadow-2xs">
            ...
            <div className="grid grid-cols-8 divide-x divide-border/70 relative">
              {/* Hour labels on left column */}
              <div className="flex flex-col divide-y divide-border/40 text-[9px] text-muted-foreground font-mono bg-muted/10 select-none">
                {Array.from({ length: gridHoursLength }, (_, i) => i + minHour).map((h) => (
                  <div
                    key={h}
                    style={{ height: `${hourHeight}px` }}
                    className="px-1 pt-0.5 flex items-start justify-center"
                  >
                    {formatHour12h(h)}
                  </div>
                ))}
              ...
              {weekDays.map((day, di) => {
                ...
                return (
                  <div
                    key={di}
                    className={`relative flex flex-col divide-y divide-border/25 transition-colors ${
                    ...
                    {Array.from({ length: gridHoursLength }, (_, h) => {
                      ...
                      return (
                        <div
                          ...
                          className={`relative group/cell cursor-pointer transition-colors ${
                            !isOutside ? "hover:bg-primary/10" : ""
                          }`}
                          title={!isOutside ? `Programar clase el ${DAY_NAMES_ES_FULL[di]} a las ${hourStr}` : undefined}
                        >
                          {!isOutside && (
