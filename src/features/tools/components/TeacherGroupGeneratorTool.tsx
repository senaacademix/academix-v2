...
import { Shuffle, ArrowLeft, Users, Sparkles, FolderOpen, AlertCircle } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { GroupGenerator } from "@/features/teacher/components/GroupGenerator";

interface TeacherGroupGeneratorToolProps {
    groups?: any[];
    onBack: () => void;
}

export function TeacherGroupGeneratorTool({ groups = [], onBack }: TeacherGroupGeneratorToolProps) {
                    ...
                    <div className="flex items-center gap-2 min-w-0">
                        ...
                        <div className="flex items-center gap-2 min-w-0">
                            <h1 className="text-xs sm:text-sm font-black text-foreground truncate tracking-tight">
                                Creador de <span className="text-primary">Grupos de Trabajo</span>
                            </h1>
            ...
            {groups.length === 0 ? (
                <div className="flex-1 rounded-3xl border border-dashed border-border/80 p-12 text-center bg-card flex flex-col items-center justify-center gap-3">
                    ...
                    <div className="space-y-1 max-w-md">
                        ...
                        <p className="text-xs text-muted-foreground">
                            Para organizar grupos de trabajo, debes tener al menos una ficha asignada con aprendices matriculados.
                        </p>
                    </div>
                </div>
                ...
                <div className="flex-1 rounded-3xl border border-dashed border-border/80 p-12 text-center bg-card flex flex-col items-center justify-center gap-3">
                    ...
                    <div className="space-y-1 max-w-md">
                        ...
                        <p className="text-xs text-muted-foreground">
                            La ficha seleccionada ({activeGroup?.name}) no tiene aprendices activos matriculados para armar equipos.
                        </p>
                    </div>
                </div>
            ) : (
